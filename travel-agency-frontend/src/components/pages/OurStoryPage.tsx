  ...
  return (
    <div className="min-h-screen pt-32 pb-20">
      <div className="container mx-auto px-4">
        ...
        {/* Story Text Section (Przywrócona karta pod zdjęciem) */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {values.map((value) => {
            ...
            return (
              <Card key={value.id} className="text-center hover:shadow-xl transition-shadow">
                <CardContent className="p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-16 text-center">
          <div>
            ...
            <div className="text-gray-600">{language === 'en' ? 'Years Experience' : 'Anni di Esperienza'}</div>
          </div>
          <div>
            <div className="text-4xl font-bold text-[#1B4965] mb-2">50K+</div>
            <div className="text-gray-600">{language === 'en' ? 'Happy Travelers' : 'Viaggiatori Felici'}</div>
          </div>
          <div>
            <div className="text-4xl font-bold text-[#1B4965] mb-2">120+</div>
            <div className="text-gray-600">{language === 'en' ? 'Destinations' : 'Destinazioni'}</div>
          </div>
          <div>
            <div className="text-4xl font-bold text-[#1B4965] mb-2">98%</div>
            <div className="text-gray-600">{language === 'en' ? 'Satisfaction Rate' : 'Tasso di Soddisfazione'}</div>
          </div>
        </div>
      </div>
    </div>
