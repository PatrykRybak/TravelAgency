...
interface DestinationCardProps {
  ...
  location: string;
  price: number;
  duration?: string; // Opcjonalny czas trwania
}
...
export function DestinationCard({
  ...
  location,
  price,
  duration = "7 days" // Domyślna wartość jeśli backend nie zwróci
  ...
  return (
    <Link to={`/travels`}>
      <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 h-full flex flex-col border-none shadow-md cursor-pointer">
        {/* Zdjęcie z efektem zoom przy hover */}
